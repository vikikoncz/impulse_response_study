...
//De ezt ugyis az interpolacional (a results tomb legyartasanal fogom megvalositani)
	...
	for(int i=0; i<x; i++){
	ki.println("%t="+tlist1[i]+"\tcurrent="+data.current_time1[0][i]*1000/par.L);
	...
	//Time-dependent results (tlist2)
	for(int i=0; i<y; i++){
	ki.println("%t="+(tlist2[i]+tlist1[z])+"\tcurrent="+data.current_time2[0][i]*1000/par.L);

	for(int j=0; j<k; j++){
		ki.print(data.x_gel[j]+"\t"+data.result_time2[0][i][j]);
		ki.print("\t"+data.result_time2[1][i][j]);
		ki.print("\t"+data.result_time2[2][i][j]);
		ki.print("\t"+data.result_time2[3][i][j]);
		ki.print("\t"+data.result_time2[4][i][j]);
		ki.println("\t"+data.result_time2[5][i][j]);
	}
